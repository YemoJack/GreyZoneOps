...
public class UIModule : IController
{
    #region 单例
    ...
    #region Resouces 加载接口
    //可在下面接口中修改为自己的资源框架加载和释放接口
    ...
    {
        //GameObject window = ResourcesLoadObj(mWindowConfig.GetWindowData(wndName).path, mUIRoot);
        GameObject window = ResourcesLoadObj(wndName, mUIRoot);
    ...
    public GameObject ResourcesLoadObj(string path, Transform parent)
    {
        ...
        //         return GameObject.Instantiate<GameObject>(obj, parent);
        // #endif
        GameObject obj = this.GetUtility<IResLoader>().LoadSync<GameObject>(path);
        GameObject panel = GameObject.Instantiate(obj, parent);
        if (panel == null)
        {
            Debug.LogError("UIModule Res UIPanel is Null");
        }
        return panel;
